...
# Finexy API Tools
# A command-line tool for calling Finexy APIs
...
$API_CONFIGS = @(
    ...
    @{
        Name = "server-version"
        Description = "Retrieve Finexy server version information"
        Method = "GET"
...
function Show-Help {
    ...
    Write-Host "Finexy API Tools"
    Write-Host ""
    Write-Host "A command-line tool for calling Finexy APIs"
    Write-Host ""
    Write-Host "Usage:"
    ...
    Write-Host "    EBKTOOL_SERVER_BASEURL      Finexy server base URL (e.g., http://localhost:8080)"
    Write-Host "    EBKTOOL_TOKEN               Finexy API token"
    Write-Host ""
...
function Invoke-Api {
    ...
    if (-not $serverBaseUrl) {
        ...
        Write-Host "Please set it to your Finexy server base URL (e.g., http://localhost:8080)"
        exit 1
    }
